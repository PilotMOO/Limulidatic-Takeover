...
public class HorseshoeCrabEntity extends Monster {
    public HorseshoeCrabEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        RandomizeSize();
    }
    public final AnimationState walkAnimationState = new AnimationState();
    public static final EntityDataAccessor<Float> Size = SynchedEntityData.defineId(HorseshoeCrabEntity.class, EntityDataSerializers.FLOAT);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, LivingEntity.class, false,
                (E) -> !(E instanceof HorseshoeCrabEntity)));
    ...
    public boolean hurt(@NotNull DamageSource source, float pAmount) {
        ...
        if (attacker instanceof HorseshoeCrabEntity){
            return false;
        }
    ...
    public static void PropagateAt(Vec3 pos, Level level){
        HorseshoeCrabEntity crab = HorseshoeEntities.HORSESHOE_CRAB.get().create(level);
        assert crab != null;
        crab.moveTo(pos);
        level.addFreshEntity(crab);
    }
    public void AlertNearby(LivingEntity newTarget){
        for (HorseshoeCrabEntity crab : level().getEntitiesOfClass(HorseshoeCrabEntity.class,
